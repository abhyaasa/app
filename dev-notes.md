...
The following setup script run completes a minimal app development installation and launches the app in the xcode emulator (which Apple calls a simulator) for testing. If some development tools are already installed, you may wish to create a modified script for execution. The voluminous output generated by the script may overflow the terminal buffer, so output is saved in `temp/setup.txt` for inspection if something goes wrong.
```
