...
The `g` script runs shortcuts in the gulp `cmdAliases` directory. For example, to initiate the most common debugging run, execute `g si`. This runs `gulp cmd si | tee temp/cmd.txt`, which runs `gulp is -i`, which runs the default gulp test build tasks and then `ionic serve -c -t ios --browser /Applications/Google\ Chrome\ Canary.app`, with output appearing on the console and saved in `temp/cmd.txt`. Run `gulp cmd` for alias list.
...
This should always be followed by updating (in this case by rebuilding) all local dependencies in `./node_modules/`, `./plugins/`, `./platforms/`.
```
$ ionic state reset
```
...
After a major Ionic version update, download new ionic image and replace `www/lib` with image version.
